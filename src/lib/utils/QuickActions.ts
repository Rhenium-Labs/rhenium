...
export default class QuickActionUtils {
	...
	public static async handleQuickMute(data: {
		...
		const successMessage =
			purgeResult && purgeResult.deleted > 0
				? `${executor}, successfully quick muted ${target} for \`${formattedDuration}\` and purged \`${purgeResult.deleted}\` ${inflect(purgeResult.deleted, "message")}.`
	...
	public static async handleQuickPurge(data: {
		...
		return Promise.all([
			...
			resultWebhook
				.send({
					content: `${executor}, successfully purged \`${purgeResult.deleted}\` ${inflect(purgeResult.deleted, "message")} from ${target}.`
				})
				.catch(() => null)
		]);
	}
