use poise::serenity_prelude::{self as serenity, ButtonStyle, CreateActionRow, CreateButton, CreateEmbed, CreateEmbedAuthor};
...
pub async fn ban(
    ...
    #[description = "The reason for the ban"] #[max_length = 1024] reason: Option<String>,
) -> Result<(), Error> {
    let data = ctx.data();
    ...
    // Check immune roles.
    {
        ...
        if !immune_roles.is_empty() {
            if let Some(target_member) = target_member.as_ref() {
                let is_immune = immune_roles.iter().any(|role| {
                    target_member
                        .roles
                        .iter()
                        .any(|r| r.to_string() == *role)
                });
                if is_immune {
    ...
    // Parse duration.
    let duration_ms = duration.as_ref().and_then(|d| crate::utils::parse_duration_string(d));
    ...
    let expires_at = duration_ms.map(|ms| chrono::Utc::now() + chrono::Duration::milliseconds(ms as i64));
    ...
    if let Some(executor_member) = executor_member.as_ref() {
        ...
        if !validation.ok {
            return reply_error(ctx, validation.message.unwrap_or_else(|| "Action not allowed.".to_string())).await;
        }
    }

    // Check if reason is required.
    if reason.is_none() && config.data.ban_requests.enforce_submission_reason {
        return reply_error(ctx, "A reason is required to submit a ban request in this server.").await;
    }
    ...
    let mut embed_fields = vec![
        ("Target".to_string(), crate::utils::user_mention_with_id(&target_id.to_string()), false),
        ("Requested By".to_string(), crate::utils::user_mention_with_id(&executor.id.to_string()), false),
    ];

    if let Some(ms) = duration_ms {
        embed_fields.insert(2, ("Duration".to_string(), crate::utils::format_duration_ms(ms), false));
    }
    ...
    let notify_content = {
        ...
        if roles.is_empty() {
            ...
            Some(
                roles.iter()
                    .map(|r| {
                        if r == "here" { "@here".to_string() } else { format!("<@&{r}>") }
                    })
                    .collect::<Vec<_>>()
                    .join(", ")
            )
    ...
    let accept_btn = CreateButton::new("ban-request-accept").label("Accept").style(ButtonStyle::Success);
    let deny_btn = CreateButton::new("ban-request-deny").label("Deny").style(ButtonStyle::Danger);
    let disregard_btn = CreateButton::new("ban-request-disregard").label("Disregard").style(ButtonStyle::Primary);
    let info_btn = CreateButton::new(format!("user-info-{}", target_id)).label("User Info").style(ButtonStyle::Secondary);
    ...
    if config.data.ban_requests.automatically_timeout {
        match target_member {
            ...
            Some(mut target_member) => {
                ...
                let timeout_until = chrono::Utc::now() + chrono::Duration::days(28) - chrono::Duration::seconds(60);
