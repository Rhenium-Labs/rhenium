...
pub async fn handle(ctx: &serenity::Context, new_message: &serenity::Message, data: &Data) {
    ...
    let queue_fut = data
        .message_manager
        .queue(new_message, &data.db, &ctx.cache);
    let highlight_fut =
        crate::commands::highlights::scan_message_for_highlights(ctx, data, new_message, guild_id);
