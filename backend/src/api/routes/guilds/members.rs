...
async fn verify_member(
    ...
    let uid: u64 = user_id.parse().map_err(|_| StatusCode::BAD_REQUEST)?;
    ...
    let in_cache = state.cache.guild(guild_id).map(|g| g.members.contains_key(&user_id)).unwrap_or(false);
