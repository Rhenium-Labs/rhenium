...
import { z } from "zod";
import { ChannelScopingType, QUICK_PURGE_CONFIG_SCHEMA } from "@repo/config";
...
const QUICK_PURGES_UPDATE_SCHEMA = z
	.object({
		enabled: z.boolean(),
		maxLimit: z.number().int().min(2).max(500),
		channelScoping: z
			.array(
				z.object({
...
export const POST: RequestHandler = async ({ request, params, locals, url }) => {
	...
	const payloadResult = QUICK_PURGES_UPDATE_SCHEMA.safeParse(payloadUnknown);
	...
	const channels = await BotApi.getGuildChannels({
		guildId: params.id,
	...
	const parsed = QUICK_PURGE_CONFIG_SCHEMA.safeParse({
		enabled: payloadResult.data.enabled,
		max_limit: payloadResult.data.maxLimit,
