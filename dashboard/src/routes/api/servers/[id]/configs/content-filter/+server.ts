...
const CONTENT_FILTER_DETECTOR_ACTION_UPDATE_SCHEMA = z
	.object({
		...
		timeoutDuration: z.string().trim().max(32).regex(CONTENT_FILTER_DURATION_REGEX)
	})
	.strict();
...
export const POST: RequestHandler = async ({ request, params, locals, url }) => {
	...
	const [channels, roles] = await Promise.all([
		BotApi.getGuildChannels({ guildId: params.id, userId: access.session.userId }),
		BotApi.getGuildRoles({ guildId: params.id, userId: access.session.userId })
	]);
