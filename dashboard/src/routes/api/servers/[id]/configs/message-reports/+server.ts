...
import { z } from "zod";
import { MESSAGE_REPORT_CONFIG_SCHEMA } from "@repo/config";
...
export const POST: RequestHandler = async ({ request, params, locals, url }) => {
	...
	const [channels, roles] = await Promise.all([
		BotApi.getGuildChannels({ guildId, userId: access.session.userId }),
		BotApi.getGuildRoles({ guildId, userId: access.session.userId })
	]);
	...
	const currentConfig = access.currentConfig;
